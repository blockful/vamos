...
export default function Markets() {
  ...
  const addresses = Array.from(
    new Set(
      markets.flatMap((m) => [m.creator, m.judge].filter((addr): addr is string => !!addr))
    )
  );
      ...
      try {
        ...
        if (values.judge && values.judge.includes('.')) {
          if (!resolvedJudgeAddress) {
            setFormError("Unable to resolve ENS name. Please check the name or use an address.");
            return;
          }
        ...
        if (!judgeAddress) {
          judgeAddress = address || "0x0000000000000000000000000000000000000000";
        }
              ...
              >
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2 text-xs text-gray-500">
                    ...
                    {market.creator && (
                      <span className="flex items-center">
                        👤 Creator: {formatAddressOrEns(market.creator, ensNames?.[market.creator], true)}
                      </span>
                    )}
                    {market.judge && (
                      <span className="flex items-center">
                        ⚖️ Judge: {formatAddressOrEns(market.judge, ensNames?.[market.judge], true)}
                      </span>
                    )}
                  </div>
                </div>
          ...
          <form onSubmit={formik.handleSubmit} className="space-y-6">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <p className="text-sm text-gray-800 font-medium">
                💡 <strong>Fee Structure:</strong> 5% of the total pot is collected as fee and split between the market creator (you) and the protocol.
              </p>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                {judgeInput && judgeInput.includes('.') && resolvedJudgeAddress && (
                  <p className="text-xs text-green-600">
                    ✓ Resolved to: {resolvedJudgeAddress.slice(0, 6)}...{resolvedJudgeAddress.slice(-4)}
                  </p>
                )}
                {judgeInput && judgeInput.includes('.') && !resolvedJudgeAddress && (
